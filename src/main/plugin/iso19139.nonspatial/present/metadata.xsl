...
<xsl:stylesheet version="2.0" xmlns:xsl ="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="metadata-iso19139.nonspatial">
...
<!--<xsl:message> GEMINI metadata.xsl NODE <xsl:value-of select="local-name(.)"/></xsl:message>-->
    ...
    <xsl:template name="iso19139.nonspatialCompleteTab">
    ...
    <xsl:template name="iso19139.nonspatial-javascript"/>

    <xsl:template name="iso19139.nonspatialBrief">
        <!-- Let the original ISO19139 templates do the work -->
  ...
  <xsl:template mode="iso19139" match="//gmd:language[gco:CharacterString]" priority="200">
    ...
    <xsl:apply-templates mode="simpleElement" select=".">
      ...
      <xsl:with-param name="text">
        <xsl:call-template name="iso19139GetIsoLanguage_nonspatial">
          <xsl:with-param name="schema" select="$schema"/>
          <xsl:with-param name="edit"   select="$edit"/>
  ...
  <xsl:template name="iso19139GetIsoLanguage_nonspatial">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
